...
direct = get_split_data_pickle_btc_noloader("DDINA")
weighted = get_split_data_pickle_btc_noloader("DDM")
...
accomplices = [
    sum([len(np.array(i.y[:, 0])) - sum(np.array(i.y[:, 0])) for i in direct[0]]),
    sum([len(np.array(i.y[:, 0])) - sum(np.array(i.y[:, 0])) for i in direct[1]]),
    sum([len(np.array(i.y[:, 0])) - sum(np.array(i.y[:, 0])) for i in direct[2]]),
]
