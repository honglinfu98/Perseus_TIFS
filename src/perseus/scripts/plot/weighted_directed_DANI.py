...
def plot_weighted_edges(
    ...
    node_size: int = 100,  # Default node size
):
    """
    Plot a weighted directed graph with customizable node size.
    """
    G1 = nx.MultiDiGraph()
    ...
    edge_widths = [G1[u][v][0]["weight"] * 8 for u, v in G1.edges()]
    nx.draw_networkx_nodes(G1, pos, node_size=node_size, node_color="skyblue")
    ...
    plt.axis("off")
    plt.savefig(path.join(PROJECT_ROOT, "data", "embedding1.png"))
    plt.show()


def plot_directed_edges(
    edge_index1: torch.Tensor, node_size: int = 100  # Default node size
):
    """
    Plot a directed graph with customizable node size.
    """
    G1 = nx.MultiDiGraph()
    ...
    plt.axis("off")
    plt.savefig(path.join(PROJECT_ROOT, "data", "embedding2.png"))
    plt.show()


if __name__ == "__main__":

    aaa, _, _ = split_data("COSS", loader=False)
    a, b, c = split_data("DDM", loader=False)
    aa, bb, cc = split_data("DDINA", loader=False)
    ...
    plot_weighted_edges(a[0].edge_index, a[0].edge_weight, node_size=800)

    plot_directed_edges(aa[89].edge_index, node_size=800)
