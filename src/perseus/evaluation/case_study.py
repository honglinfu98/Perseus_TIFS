...
features_buffer = combine_feature["POWR"]
features_buffer = features_buffer[
    features_buffer["telegram_chat_id"].isin(gs["POWR"].nodes)
]
...
id_to_index = {
    ...
    for index, telegram_chat_id in enumerate(features_buffer["telegram_chat_id"])
}

# Create edge index
edge_index = []
for source_node, target_node in gs["POWR"].edges():
...
for node_id in features_buffer["telegram_chat_id"]:
    node_labels = label_mapping["POWR"].get(node_id, 0)
...
with torch.no_grad():
    ...
    for i, pred in enumerate(predictions.cpu().numpy()):
        node_id = node_ids[i]
        actual_label = label_mapping["POWR"].get(
            node_id, 0
