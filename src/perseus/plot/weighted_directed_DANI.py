...
def plot_graphs_side_by_side(edge_index1, edge_weight1,  title1='Graph 1', title_size=16):
    ...
    # # Convert tensors if necessary and add edges for Graph 2
    # if isinstance(edge_index2, torch.Tensor):
    #     edge_index2 = edge_index2.t().tolist()
    # edge_weights2 = edge_weight2.tolist()
    # for (u, v), weight in zip(edge_index2, edge_weights2):
    ...
    nx.draw_networkx_edges(G1, pos, width=edge_widths, arrowstyle='-|>', arrowsize=50, edge_color='k', connectionstyle='arc3, rad = 0.1')
    ...
    # nx.draw_networkx_nodes(G2, pos, node_size=1200, node_color='skyblue')
    # nx.draw_networkx_edges(G2, pos, arrowstyle='-|>', arrowsize=50, edge_color='k')
...
plot_graphs_side_by_side(aaa[0].edge_index, aaa[0].edge_weight, title1='Weighted DANI Network Graph', title_size=40)


# # Store original indexes with the elements
# indexed_data_list = [(index, data) for index, data in enumerate(a)]

# # Sort the data using calculate_ratio applied to the elements of indexed_data_list
# sorted_indexed_data_list = sorted(indexed_data_list, key=lambda x: calculate_ratio(x[1]), reverse=True)

# # Retrieve the original indexes after sorting
# original_indexes = [item[0] for item in sorted_indexed_data_list]
# sorted_data_list = [item[1] for item in sorted_indexed_data_list]

# # Now you can use original_indexes to refer to the original positions before sorting,
# # and sorted_data_list contains your data sorted as per your custom key function.
# print("Original indexes after sorting:", original_indexes)
