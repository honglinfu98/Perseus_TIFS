...
from perseus.settings import PROJECT_ROOT
from perseus.dataset.preprocess.new_process import get_new_graphs
from perseus.dataset.preprocess.process import (
    aggregate_data,
    assign_event_ids,
    process_dataframe,
...
def prepare_data(graphs, features, label_mapping, P_dict):
    prepared_data = []
    for key, graph in graphs.items():
        # Extract features for nodes present in the graph
        features_buffer = features[key]
        ...
        for (source_node, target_node), weight in P_dict[key].items():
            if weight > 0:  # Ensure both nodes are in the id_to_index mapping
                edge_index_list.append(
                    [id_to_index.get(source_node), id_to_index.get(target_node)]
                )
                edge_weight_list.append(weight)
        # Convert lists to PyTorch tensors
        print(key)
...
def get_undirect_weighted_data_loader():
    train_signals = get_train_scored_signals()
    processed_train_signals = process_dataframe(train_signals)
    ided_train_signals = assign_event_ids(processed_train_signals)
    train_cascade, train_no_nodes, train_id_mapping = aggregate_data(ided_train_signals)
    train_gs, results, As, P_dict_train = get_new_graphs(
        train_cascade, train_no_nodes, train_id_mapping
    )
    train_f = graph_features(train_gs)
    train_market_features = features_engineer(processed_train_signals)
    ...
    ided_signals = assign_event_ids(processed_signals)
    cascade, no_nodes, id_mapping = aggregate_data(ided_signals)
    gs, results, As, P_dict = get_new_graphs(cascade, no_nodes, id_mapping)
    f = graph_features(gs)
    market_features = features_engineer(processed_signals)
    c = combine_features(market_features, f)
    # ngs = get_similar_graph(c)
    ...
    train_data = prepare_data(
        train_gs, train_market_features, train_label_mapping, P_dict_train
    )
    test_data = prepare_data(gs, market_features, label_mapping, P_dict)
...
if __name__ == "__main__":
    a, b = get_undirect_weighted_data_loader()
...
# from clotho.settings import PROJECT_ROOT
# from clotho.dataset.preprocess.new_process import get_new_graphs
# from clotho.dataset.preprocess.process import (
#     aggregate_data,
#     assign_event_ids,
#     process_dataframe,
...
#     # Prepare data for training and testing sets
#     train_data = prepare_data(train_gs, train_market_features, train_label_mapping, P_dict_train)
#     test_data = prepare_data(gs, market_features, label_mapping, P_dict)
