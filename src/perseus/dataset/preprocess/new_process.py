...
def get_new_graphs(cascade: dict, no_nodes: dict, id_mapping: dict):
    """
    This function creates the graphs for the cascadeX and updates P_dict's indices according to id_mapping.
    :param cascade: the cascade
    :param no_nodes: the number of nodes for each commodity
    :param id_mapping: the mapping between the commodity and the id
    :return: the graphs, result, A, and updated P_dict
    """
    ...
    for key, value in ensure_graph_learned.items():
        # Assuming DANI function returns graph, result, A, and original P_dict
        graphs[key], result[key], A[key], original_P_dict = DANI(
            ensure_graph_learned[key], cascade[key]
        )
        # Relabel graph nodes
        graphs[key] = nx.relabel_nodes(graphs[key], id_mapping[key]["new_to_id"])

        # Update P_dict indices according to id_mapping
        new_P_dict = {}
        for (i, j), prob in original_P_dict.items():
            new_i = id_mapping[key]["new_to_id"].get(i)
            new_j = id_mapping[key]["new_to_id"].get(j)
            if new_i is not None and new_j is not None:
                new_P_dict[(new_i, new_j)] = prob
        P_dict[key] = new_P_dict
