...
def get_btc_train_scored_signals():
    ...
    with open(
        path.join(PROJECT_ROOT, "data", "old", "train_signals_btc.pkl"), "rb"
    ) as file:
        signals = pickle.load(file)
    result = signals[~signals["telegram_chat_id"].isna()]
...
def get_btc_valid_scored_signals():
    ...
    with open(
        path.join(PROJECT_ROOT, "data", "old", "valid_signals_btc.pkl"), "rb"
    ) as file:
        signals = pickle.load(file)
    result = signals[~signals["telegram_chat_id"].isna()]
...
def get_btc_test_scored_signals():
    ...
    with open(
        path.join(PROJECT_ROOT, "data", "old", "test_signals_btc.pkl"), "rb"
    ) as file:
        signals = pickle.load(file)
    result = signals[~signals["telegram_chat_id"].isna()]
